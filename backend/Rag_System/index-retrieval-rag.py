...
# Mongo globals (initialized lazily)
mc = MongoClient(MONGO_URI)
db = mc[MONGO_DB]


def init_mongo(raise_on_fail: bool = True, timeout_ms: int = 5000):
    """Initialize global Mongo client and DB. Accepts common env var names.
    Returns tuple (mc, db) or (None, None) on failure when raise_on_fail is False.
    """
    global mc, db, MONGO_URI, MONGO_DB
    # Accept multiple env var names for compatibility
    MONGO_URI = MONGO_URI or os.getenv("MONGODB_URI") or os.getenv("MONGO_URL")
    MONGO_DB = MONGO_DB or os.getenv("MONGODB_DB") or os.getenv("MONGO_DB") or MONGO_DB

    if not MONGO_URI:
        msg = "MONGO_URI (or MONGODB_URI) environment variable is not set."
        if raise_on_fail:
    ...
    try:
        mc = MongoClient(MONGO_URI, serverSelectionTimeoutMS=timeout_ms)
        # quick ping
        mc.admin.command('ping')
        db = mc[MONGO_DB]
        LOGGER.info("✅ Successfully connected to MongoDB at %s (DB=%s)", MONGO_URI, MONGO_DB)
        return mc, db
    except ServerSelectionTimeoutError as e:
        msg = f"❌ MongoDB server selection timed out: {e}"
        if raise_on_fail:
            raise RuntimeError(msg)
        LOGGER.error(msg)
        return None, None
...
def chunk_text_atomic(text: str, prefix: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    for i, s in enumerate(sentences):
        ...
        chunks.append({
            "chunk_id": chunk_id,
            "text": s,
            "metadata": {"source": prefix or "file"}
        })
    return chunks
...
if __name__ == "__main__":
    ...
    p.add_argument('--skip-mongo', action='store_true', help='Skip connecting to MongoDB and only index files')
    p.add_argument('--query', type=str, help='Run a quick query')
    args = p.parse_args()
    if args.index_all:
        if not args.skip_mongo:
            try:
                mc_ret, db_ret = init_mongo()
            except Exception as e:
                LOGGER.error('Mongo initialization failed: %s', e)
                LOGGER.info('Skipping Mongo indexing due to failure; use --skip-mongo to ignore errors')
                mc_ret = None
                db_ret = None
            if db_ret is not None:
                # index mongo
                for c in db_ret.list_collection_names():
                    index_collection(c)
            else:
                LOGGER.info('No Mongo DB available; skipped collection indexing')
        else:
            LOGGER.info('Skipping Mongo indexing as requested (--skip-mongo)')
