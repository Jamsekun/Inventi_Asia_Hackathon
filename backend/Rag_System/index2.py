...
def chunk_doc_atomic(collection_name, doc):
    """
    Convert a MongoDB document or Markdown file into multiple small chunks suitable for RAG.
    ...
    def add_chunk(text, metadata_extra=None):
        # For non-rules collections, base_id comes from doc
        ...
        print(f"[CHUNK] Added: {chunk_id} | Section: {metadata_extra.get('section')} | Field: {metadata_extra.get('field')}")

    try:
        if collection_name == "rules":
            # Always load from the fixed rules.md file
            if not os.path.exists(RULES_FILE):
                raise FileNotFoundError(f"Rules file not found at {RULES_FILE}")

            with open(RULES_FILE, "r", encoding="utf-8") as f:
                content = f.read()
            print(f"[INFO] Loaded rules.md from {RULES_FILE}")
            ...
            for part in sections:
                if part.strip().startswith("#"):
                    ...
                    print(f"[SECTION] Processing under: {current_heading}")
