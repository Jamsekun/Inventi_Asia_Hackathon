...
import { sendChat } from "./chat_api"; // Import the backend function
import "./index.css";
...
async function handleSend(text) {
  // 1. Guard clause: ignore empty or whitespace-only input
  if (!text || !text.trim()) return;

  // 2. Create a new user message object with frontend-only fields
  const msg = {
    id: Date.now(),
    text: text.trim(), // used for display
    role: "user",
    time: timeNow(),    // custom timestamp function
  };

  // 3. Optimistically add the user message to the active conversation
  setConversations((prev) => {
    const copy = [...prev];
  ...
  // 4. Set typing indicator before sending request
  setIsTyping(true);

  try {
    // 5. Get the latest conversation state (after adding user message)
    const currentConversation = conversations[activeIndex];
    const fullMessages = [...currentConversation.messages, msg];

    // 6. Format messages for backend: strip frontend-only fields
    const backendMessages = fullMessages.map((m) => ({
      role: m.role,
      content: m.text || m.content, // ensure compatibility
    }));

    // 7. Send to backend and await response
    const request = await sendChat(backendMessages); // returns { intent, response, relevant_data }

    // 8. Create assistant reply object for frontend
    ...
    // 9. Append assistant reply to conversation
    setConversations((prev) => {
      const copy = [...prev];
      const target = { ...copy[activeIndex] };
  ...
  // 11. Scroll to bottom of chat after rendering
  setTimeout(() => {
      ...
      <div className="inventi-root">
        <div className="inventi-layout">
          ...
          <main className="inventi-main" aria-hidden={widgetOpen ? "true" : "false"}>
            ...
            <div className="input-bar" style={{ padding: "8px" }}>
              <MessageInput
